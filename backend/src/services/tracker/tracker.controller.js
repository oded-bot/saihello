...
function getUpcomingEvent(req, res) {
  try {
    const event = db.prepare('SELECT * FROM upcoming_events WHERE id = ?').get(req.params.id);
    if (!event) return res.status(404).json({ error: 'Event nicht gefunden' });
    res.json(event);
  } catch (err) {
...
function getUpcomingActive(req, res) {
  try {
    const event = db.prepare('SELECT * FROM upcoming_events WHERE id = ?').get(req.params.id);
    if (!event) return res.status(404).json({ error: 'Event nicht gefunden' });
...
function registerForUpcoming(req, res) {
  try {
    const { name, email, city, upcomingEventId, referredBy } = req.body;
    if (!name?.trim() || !email?.trim() || !upcomingEventId) {
      return res.status(400).json({ error: 'Name, E-Mail und Event sind erforderlich' });
    }

    const event = db.prepare('SELECT * FROM upcoming_events WHERE id = ?').get(upcomingEventId);
    if (!event) return res.status(404).json({ error: 'Event nicht gefunden' });
    ...
    const prevCount = db.prepare('SELECT COUNT(*) as cnt FROM tracker_registrations WHERE upcoming_event_id = ?').get(upcomingEventId).cnt;
    const refCode = getUniqueRefCode();
    let isNew = true;

    try {
      db.prepare(`
        INSERT INTO tracker_registrations (upcoming_event_id, name, email, city, referral_code, referred_by)
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(upcomingEventId, name.trim(), email.trim().toLowerCase(), city?.trim() || null, refCode, referredBy || null);
    } catch (err) {
      if (!err.message.includes('UNIQUE')) throw err;
      isNew = false;
    }

    const newCount = db.prepare('SELECT COUNT(*) as cnt FROM tracker_registrations WHERE upcoming_event_id = ?').get(upcomingEventId).cnt;
    const reg = db.prepare('SELECT id, referral_code FROM tracker_registrations WHERE upcoming_event_id = ? AND email = ?').get(upcomingEventId, email.trim().toLowerCase());
    const referralCount = db.prepare('SELECT COUNT(*) as cnt FROM tracker_registrations WHERE upcoming_event_id = ? AND referred_by = ?').get(upcomingEventId, reg.referral_code).cnt;
    const milestoneCelebration = isNew ? (MILESTONES.find(m => m <= newCount && m > prevCount) || null) : null;
...
function getBySlug(req, res) {
  try {
    const event = db.prepare('SELECT * FROM upcoming_events WHERE slug = ?').get(req.params.slug);
    if (!event) return res.status(404).json({ error: 'Event nicht gefunden' });

    const count = db.prepare('SELECT COUNT(*) as cnt FROM tracker_registrations WHERE upcoming_event_id = ?').get(event.id).cnt;
    const recentCount = db.prepare(`SELECT COUNT(*) as cnt FROM tracker_registrations WHERE upcoming_event_id = ? AND created_at >= datetime('now', '-48 hours')`).get(event.id).cnt;
    const recentNames = db.prepare(`SELECT name FROM tracker_registrations WHERE upcoming_event_id = ? ORDER BY created_at DESC LIMIT 6`).all(event.id).map(r => r.name.split(' ')[0]);
    const cities = db.prepare(`SELECT city, COUNT(*) as cnt FROM tracker_registrations WHERE upcoming_event_id = ? AND city IS NOT NULL AND city != '' GROUP BY city ORDER BY cnt DESC LIMIT 8`).all(event.id);
    ...
    res.json({
      active: true,
      event: { ...event, threshold_hard: thresholdHard, threshold_soft: thresholdSoft },
      count, recentCount, recentNames, cities, phase, nextMilestone,
      thresholdReached: count >= thresholdHard,
      softThresholdReached: count >= thresholdSoft,
    });
  } catch (err) {
...
module.exports = { getActive, register, getUpcomingEvent, getUpcomingActive, registerForUpcoming, getFeatured, getBySlug };
